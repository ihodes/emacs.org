...
** Main
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (That's why I add 6 to the DOW, to shift things over 6, to Monday)."
        (interactive)
        ;; TODO: This opens up a note for the current day; we really want last
        ;;       Monday's date.
    ...
    (use-package visual-fill-column
      :init
      (setq fill-column 80)
      (defun ihodes/activate-visual-line-mode-for-journal ()
        "Activate soft line-wrapping when inside a current journal entry; this lets us view them nicely on a mobile device as well as on desktop."
        (when (and (string-match "db/journal/" (buffer-file-name))
                   (string-match "\\.md\\'" (buffer-file-name)))
          (progn (visual-fill-column-mode t)
                 (visual-line-mode t))))
      (add-hook 'find-file-hook 'ihodes/activate-visual-line-mode-for-journal))
  #+END_SRC
