  ...
  (use-package markdown-mode
    ...
    (defun s-trim-right (s)
      ...
      (if (string-match "[ \t\n\r]+\\'" s)
          (replace-match "" t t s)
        s))
    (defmacro ihodes/make/start-new-journal-entry
        (locations-spec)
      (append
       (list 'defhydra 'ihodes/hydra/start-new-journal-entry '(:color blue) "Where are you now?")
       (mapcar '(lambda (spec)
                  (list (nth 0 spec)
                        (list 'ihodes/start-new-journal-entry (nth 1 spec))
                        (nth 2 spec)))
               (eval locations-spec))
       (list (list "c" nil "cancel"))))
    ;; This is so that the location of e.g. my house isn't stored in Github
    (ihodes/make/start-new-journal-entry (read-string-from-file "~/workspace/emacs.org/locations.el"))
    (defun ihodes/start-new-journal-entry (location)
      (let ((location (if (stringp location)
                          (concat ": " location)
                        (if location
                            (concat ": " (read-string "Where are you? "))
                          ""))))
        (insert "# ")
        (org-time-stamp 1)
        (newline)
